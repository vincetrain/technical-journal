...
```
>This solves error status 503, `"master_not_discovered_exception"`. Ensure `node.name` and the node name in `cluster.initial_master_nodes` match.
...
Modify the `modules.d/logstash-xpack.yml/` file to include authentication.
```yaml
- module: logstash
  xpack.enabled: true
  period: 10s
...
Modify `metricbeat.yml` to output to Elasticsearch.
```yml
output.elasticsearch:
	hosts: ["https://url-to-elasticsearch:9200":]
	preset: balanced
	ssl.certificate_authorities: /path/to/http_ca.crt
	ssl.certificate: /path/to/metricbeat.crt
	ssl.key: /path/to/metricbeat.key
	protocol: "https"
	username: "metricbeat"
	password: "supersecurepassword"
```
> This requires the creation of a new certificate for Metricbeat, alongside a new user with the `remote_monitoring_collector` role (refer to [Installing & Configuring Kibana](#Installing%20&%20Configuring%20Kibana)).
> `http_ca.crt` is the same `http_ca.crt` that Elasticsearch autogenerated.
### Logstash Pipelines
...
```
> Also consider adding the `logstash` group to the user who will be running Logstash if running via its binary (requires re-login/reboot to work). Refer to [Installing & Configuring Kibana](#Installing%20&%20Configuring%20Kibana)
...
Enable delivery of monitoring data with SSL to Elasticsearch in `filebeat.yml`
```yaml
monitoring:
	enabled: true
	...
	elasticsearch:
		...
		ssl.key: "/path/to/filebeat.key"
```
> `http_ca.crt` is the same certificate autogenerated by Elasticsearch.
> Credentials must authenticate to a user who has the `beats_system` role.
> To create a user with this role, refer to [Installing & Configuring Kibana](#Installing%20&%20Configuring%20Kibana)
